...
/** Monorepo npm pode hoistar `socket.io-client` só na raiz; o alias evita falha do pré-bundle do Vite. */
function resolveSocketIoClientDir(): string {
  try {
    return path.dirname(require.resolve('socket.io-client/package.json'))
  } catch {
    throw new Error(
      'Dependência `socket.io-client` não encontrada. Na raiz do repositório execute: npm install',
    )
  }
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    resolve: {
      alias: {
        'socket.io-client': resolveSocketIoClientDir(),
      },
    },
