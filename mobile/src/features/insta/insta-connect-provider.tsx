...
export function InstaConnectProvider({ children }: { children: ReactNode }) {
  ...
  const [sessions, setSessions] = useState<InstaSessionItem[]>([])
  ...
  const createSession = useCallback(async (setAsActive = true): Promise<InstaSessionsResult> => {
    setIsManagingSessions(true)
    try {
    ...
    async (sessionId: string, username: string, password: string): Promise<InstaLinkResult> => {
      ...
      try {
        ...
        if (data.ok && !data.success) {
          return {
            success: false,
            error: data.message ?? "O Instagram não confirmou o login. Verifique as credenciais ou 2FA.",
          }
        }
